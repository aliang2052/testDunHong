...
function buildWorld(scene) {
  ...
  /* ---------------- 沙丘顶 ---------------- */
  {
    const g = new THREE.PlaneGeometry(360, 240, 160, 110);
    g.rotateX(-Math.PI / 2);
    const pa = g.attributes.position;
    for (let i = 0; i < pa.count; i++) {
      const x = pa.getX(i);
...
function makeWallMaterial(uvScale = [4, 4]) {
  ...
  mat.onBeforeCompile = (sh) => {
    ...
    sh.fragmentShader = `
      ...
      float pw(float k){ return clamp(uWallPhase - k, 0.0, 1.0); }
    ` + sh.fragmentShader;
...
function setSectionX(x) { CARVE_U.uSectionX.value = x; setWallSection(x); }
