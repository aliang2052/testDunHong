...
const APP = {
  // 首屏展示第一章中已经露出主体的静态预览；首次点击播放或章节 01
  // 仍会回到 0 秒完整播放揭幕过程。
  time: 2.5,
  playing: false,
  initialPreview: true,
...
function bindUI() {
  ...
  addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowLeft') { APP.initialPreview = false; APP.time = Math.max(0, APP.time - 3); APP.playUntil = null; APP.playFromChapter = -1; APP.dirty = true; }
    if (e.key === 'ArrowRight') { APP.initialPreview = false; APP.time = Math.min(DURATION, APP.time + 3); APP.playUntil = null; APP.playFromChapter = -1; APP.dirty = true; }
  });
...
function installDebugAPI() {
  window.MOGAO = {
    ...
    play() {
      if (APP.initialPreview) { APP.initialPreview = false; APP.time = 0; renderFrame(APP.time, 0); }
      APP.playUntil = null; APP.playFromChapter = -1; APP.playing = true; APP.dirty = true; elPlay.textContent = '⏸';
    },
