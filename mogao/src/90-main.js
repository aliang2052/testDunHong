...
function init() {
  ...
  camera = new THREE.PerspectiveCamera(35, 9 / 16, 0.42, 1400);
  freeCam = new THREE.PerspectiveCamera(45, 9 / 16, 0.42, 1400);
...
/* ------------------------------------------------------------
   画布尺寸：竖屏 9:16，与参考视频构图一致
   ------------------------------------------------------------ */
function onResize() {
  const wrap=document.getElementById('stage');
  const aw=Math.max(1,wrap.clientWidth), ah=Math.max(1,wrap.clientHeight), aspect=9/16;
