...
当前代码以 ChatGPT Pro 的 **Round 2** 时间轴和 9:16 竖屏构图为基线，重新实现了佛像近景。网页通过程序化几何、PBR 材质、粒子、施工工具和确定性时间轴展示栈道搭建、洞窟开凿、石胎塑形、逐层敷泥、收光、彩绘、壁面装饰和九层楼复原，不嵌入或播放原视频。

当前版本包含 16 个可点击、可独立播放的施工章节，支持暂停、倍速、进度拖拽和桌面自由视角。全部运行资源内联，可离线打开。

佛像的头部、五官、双手、螺发、衣袖、袈裟和衣褶均为带法线的实体三维网格；程序化 Canvas 纹理只承担材料微细节，不承载人物轮廓或照片。自动化几何、视差、功能、离线和响应式门禁均已通过。完整结论和逐帧证据见 [真实 3D 佛像验收](docs/REAL_3D_BUDDHA_VALIDATION.md)。程序化模型与参考视频仍存在可见写实度差距，不能把工程门禁通过表述成扫描级或 100% 视觉复刻。
...
- `mogao/src/42-buddha-realism.js`：佛像头脸、双手、螺发、衣袖与袈裟的实体三维升级层
...
- `mogao/verify-buddha-3d.js`：三维几何、法线、面数和多视角视差门禁
...
- 当前单文件 HTML：1,584,543 bytes
- 当前单文件 SHA-256：`3b94ed50e201bb092c9a175571176121afcf161286f23a37896d1b988365e219`
- 原视频不进入 Git 历史。
